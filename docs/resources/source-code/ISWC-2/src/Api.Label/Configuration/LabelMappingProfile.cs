...
namespace SpanishPoint.Azure.Iswc.Api.Label.Configuration
{
    internal class LabelMappingProfile : Profile
    {
        public LabelMappingProfile() {
            CreateMap<V1.Submission, SubmissionModel>()
                 .ForMember(dest => dest.WorkNumber, opt => opt.MapFrom((src, dest) =>
                 {
                     ...
                     byte[] codeBuffer = RandomNumberGenerator.GetBytes(32);
                     int r = RandomNumberGenerator.GetInt32(10, 16);
                     var randomCode = Convert.ToBase64String(codeBuffer).Substring(0, r).Replace("+", "").Replace("/", "").Replace("=", "");
                 ...
                 .ForMember(dest => dest.SourceDb, opt => opt.MapFrom(o => 312))
                         ...
                         if (src.AdditionalIdentifiers.Recordings != null)
                         {
                             foreach (var rec in src.AdditionalIdentifiers.Recordings)
                             {
                                 list.Add(new AdditionalIdentifier
                                 {
                                     ...
                                     Performers = rec.Performers != null && rec.Performers.Count() > 0 ? rec.Performers.Select(x => new Performer
                                     {
                                         Isni = x.Isni,
                                         Ipn = (int?)x.Ipn,
