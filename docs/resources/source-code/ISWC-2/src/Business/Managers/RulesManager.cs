...
namespace SpanishPoint.Azure.Iswc.Business.Managers
{
    ...
    public class RulesManager : IRulesManager
    {
        ...
        public async Task<IEnumerable<T>> GetEnabledRules<T>() where T : IBaseRule
        {
            ...
            {
                var service = serviceProvider.GetService(rule);
        ...
        {
            var cached = await memoryCache.GetOrCreateAsync<IDictionary<string, string>>(
                "Rules",
                async rules =>
                {
                    rules.Size = 10;
                    ...
                    var raw = await rulesService.GetEnabledRules();

                    IEnumerable<ParameterModel> enabledRules =
                        raw == null
                            ? Array.Empty<ParameterModel>()
                            : raw.Where(r => !string.IsNullOrEmpty(r.Value));
