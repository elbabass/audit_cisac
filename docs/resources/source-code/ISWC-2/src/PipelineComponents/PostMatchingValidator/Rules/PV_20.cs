    ...
    public class PV_20 : IRule, IAlwaysOnRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            bool CheckTitles(IEnumerable<Title> subTitles, MatchingWork? match, ISWCMetadataModel? metadataModel, IEnumerable<TitleType> titleExclusions)
            {
                ...
                if (match == null && (metadataModel == null || !metadataModel.Titles.Any())) 
                    return false;
                ...
                else if (paramValue)
                {
                    return match != null ? StringExtensions.StringComparisonExact(submission.MatchedResult.StandardizedName, match?.StandardizedTitle)
                        : (matchTitles ?? new List<Title>()).Any(x => (subTitles.Any(y => StringExtensions.StringComparisonExact(y.Name, x?.Name)
                ...
                else
                {
                    ...
                    if (subTitles.Where(x => !titleExclusions.Contains(x.Type)).Any(
                       t => !(matchTitles ?? new List<Title>()).Where(ip => (StringExtensions.StringComparisonExact(t.Name, ip.Name)
                    ...
                    else return (matchTitles ?? new List<Title>()).Where(x => !titleExclusions.Contains(x.Type)).Any(
