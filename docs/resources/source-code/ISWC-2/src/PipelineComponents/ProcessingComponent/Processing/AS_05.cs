...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.ProcessingComponent.Processing
{
    public class AS_05 : IProcessingSubComponent
    {
        ...
        public async Task<IswcModel> ProcessSubmission(Submission submission)
        {
            ...
            if (submission.IsEligibileOnlyForDeletingIps &&
                !submission.IswcModel.VerifiedSubmissions.First(x => x.WorkNumber.Number == submission.Model.WorkNumber.Number).IswcEligible)
                return submission.IswcModel;

            if (submission.IsPortalSubmissionFinalStep)
            {
                submission.Model.IswcsToMerge = new List<string>() { IswcToMerge };
                submission.Model.PreferredIswc = submission.MatchedResult.Matches.First().Numbers.First(x => x.Type == "ISWC"
                    && x.Number.Equals(submission.Model.PreferredIswc)).Number ?? string.Empty;
                submission.IswcModel.VerifiedSubmissions.First().LinkedTo = IswcToMerge;
            }
            else
            {
                ...
                {
                    submission.Model.IswcsToMerge = submission.MatchedResult.Matches.Where(x => x.IswcStatus == 2).Select(x => x.Numbers.First(n => n.Type == "ISWC").Number).ToList();
                }

                if (submission.RequestType == RequestType.Label && submission.IsrcMatchedResult.Matches.Where((x => x.IswcStatus == 1)).Any())
                {
                    submission.Model.PreferredIswc = submission.IsrcMatchedResult.Matches.First(x => x.IswcStatus == 1).Numbers.First(x => x.Type == "ISWC").Number ?? string.Empty;
                }
                else
                {
                    submission.Model.PreferredIswc = submission.MatchedResult.Matches.First().Numbers.First(x => x.Type == "ISWC").Number ?? string.Empty;
                }

                submission.IswcModel.VerifiedSubmissions.First().LinkedTo = submission.Model.PreferredIswc;
            }
            ...
            return (await workManager.FindManyAsync(new long[] { newWorkInfoId }, detailLevel: submission.DetailLevel)).First();
