...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.StaticDataValidator.Rules
{
    public class IV_25 : IRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            {
                if (interestedParty.Names != null && interestedParty.Names.Any(i => ipsToIgnore.SelectMany(x => x.Names ?? Enumerable.Empty<NameModel>()).Select(x => x.IpNameNumber).Contains(i.IpNameNumber)))
                {
