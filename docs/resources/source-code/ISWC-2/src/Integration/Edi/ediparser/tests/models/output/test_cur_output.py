...
def test_CUR_get_record():
    ...
    cur_records = CUR(api_response.json(), None, None).get_record()
    ...
    assert expected_value == cur_records


def test_CUR_get_child_records():
    api_response = Response()
    ...
    cur_childrecords = CUR(api_response.json(), None,
                           None).get_child_records()

    expected_value = ['CWI                315string              315T9031238434           ',
                      'CIP                463130289  I-002234809-8C ']

    assert expected_value == cur_childrecords
