...
Submission is derived from two works.
In file is in JSON format.
"""


def test_add_derived_submission_json(sftp_client, cosmos_service, databricks_client):
    api = IswcApi()
    ...
    derived_submission = EligibleSubmission(0)
    derived_submission.add_derived_from_works([iswc_one, iswc_two])
    filename = generate_json_allocation_in_file([derived_submission])
...
def test_add_derived_submission_tsv(sftp_client, cosmos_service, databricks_client):
    api = IswcApi()
    submission_one = EligibleSubmission(0)
    submission_two = EligibleSubmission(0)
    ...
    derived_submission = EligibleSubmission(0)
    derived_submission.add_derived_from_works([iswc_one, iswc_two])
    filename = generate_tsv_allocation_in_file([derived_submission])
