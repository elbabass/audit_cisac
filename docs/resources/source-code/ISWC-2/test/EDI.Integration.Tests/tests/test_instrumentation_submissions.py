...
Submission is disambiguated and contains Instrumentations.
In file is in JSON format.
"""


def test_add_instrumentation_submission_json(sftp_client, cosmos_service, databricks_client):
    api = IswcApi()
    ...
    disambiguated_submission.add_instrumentation('BAR')
    disambiguated_submission.add_instrumentation('BNJ')
...
def test_add_instrumentation_submission_tsv(sftp_client, cosmos_service, databricks_client):
    api = IswcApi()
    submission_one = EligibleSubmission(0)
