...
using SpanishPoint.Azure.Iswc.Api.ThirdParty.Integration.Tests.Configuration;
using SpanishPoint.Azure.Iswc.Api.ThirdParty.Integration.Tests.Extensions;
using System;
...
namespace SpanishPoint.Azure.Iswc.Api.ThirdParty.Integration.Tests.V1
{
    public abstract class TestBase
    {
        ...
        public static async Task<HttpClient> GetMatchingEngineClient()
        {
            ...
            {
                Address = new Uri($"{baseUrl}connect/token").AbsoluteUri,
                ClientId = config.MatchingEngineMicrosoftEntraIDClientID,
                ClientSecret = config.MatchingEngineMicrosoftEntraIDClientSecret,
                Scope = $"{config.MatchingEngineMicrosoftEntraIDScope}/.default"
            });
            client.SetBearerToken(response.AccessToken);
            return client;
        }
