...
namespace SpanishPoint.Azure.Iswc.Api.Publisher.Integration.Tests.V1.Tests.SearchTests
{
    ...
    public class ContextSearchTests : TestBase, IClassFixture<ContextSearchTests_Fixture>
    {
        ...
        public async void ContextSearchTests_WithLastNameAndNameNumber_ReturnsSuccess()
        {
            ...
            Assert.Equal(ip.LastName, firstResult.LastName);
            Assert.Equal(ip.NameNumber, firstResult.NameNumber);
            Assert.NotEmpty(firstResult.MatchingIswcs);
            Assert.Single(firstResult.MatchingIswcs);
            var matchingIswc = firstResult.MatchingIswcs.First();
        ...
        public async void ContextBatchSearchBatchTests_WithMultipleSubmissions_ReturnsSuccess()
        {
            ...
            Assert.NotEmpty(firstBatchResult.SearchResults);
            var firstSearchResult = firstBatchResult.SearchResults.First();
            Assert.Equal(submissionOne.InterestedParties.First().LastName, firstSearchResult.LastName);
            Assert.Contains(firstSearchResult.MatchingIswcs, iswc => iswc.OriginalTitle == submissionOne.OriginalTitle);
            Assert.Null(firstBatchResult.Rejection);

            var secondBatchResult = batchRes.Skip(1).First();
            Assert.NotEmpty(secondBatchResult.SearchResults);
            var secondSearchResult = secondBatchResult.SearchResults.First();
            Assert.Equal(submissionTwo.InterestedParties.First().LastName, secondSearchResult.LastName);
            Assert.Contains(secondSearchResult.MatchingIswcs, iswc => iswc.OriginalTitle == submissionTwo.OriginalTitle);
        ...
        public async void ContextSearchBatchTests_WithMultipleSubmissions_ReturnsMultipleResponse()
        {
            ...
            var secondBatchResult = batchRes.Skip(1).First();
            Assert.Empty(secondBatchResult.SearchResults);
