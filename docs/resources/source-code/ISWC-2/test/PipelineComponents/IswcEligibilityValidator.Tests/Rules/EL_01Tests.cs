...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.IswcEligibilityValidator.Tests.Rules
{
    ...
    public class EL_01Tests
    {
        ...
        public async Task EL_01_EligibleOneIpRepresentedBySubmitter()
        {
            ...
            rulesManagerMock.Setup(v => v.GetParameterValue<string>("IncludeAgenciesInEligibilityCheck")).ReturnsAsync("345(030,098);308(031,062,086,093,219,201,189,066)");
            agreementManagerMock.Setup(a => a.FindManyAsync(It.IsAny<IEnumerable<string>>())).ReturnsAsync(new List<Agreement>() {
                new Agreement { Agency = new Agency { AgencyId = "031" }, IpbaseNumber = "777" } });

            var test = new Mock<EL_01>(rulesManagerMock.Object, agreementManagerMock.Object).Object;
            ...
            Assert.Equal(nameof(EL_01), test.Identifier);
        ...
        /// </summary>
        [Fact]
        public async Task EL_01_EligibleOneIPRepresentedBySubmitterRoleTypeIneligible()
        {
            var submission = new Submission() { Model = new SubmissionModel() };
            var rulesManagerMock = new Mock<IRulesManager>();
            var agreementManagerMock = new Mock<IAgreementManager>();
        ...
        /// </summary>
        [Fact]
        public async Task EL_01_Ineligible()
        {
            var submission = new Submission() { Model = new SubmissionModel() };
            var rulesManagerMock = new Mock<IRulesManager>();
            var agreementManagerMock = new Mock<IAgreementManager>();
            ...
            rulesManagerMock.Setup(v => v.GetParameterValue<string>("IncludeAgenciesInEligibilityCheck")).ReturnsAsync("345(030, 098);308(030, 062, 086, 093, 219, 201, 189, 066)");
            agreementManagerMock.Setup(a => a.FindManyAsync(It.IsAny<IEnumerable<string>>())).ReturnsAsync(new List<Agreement>() {
                new Agreement { Agency = new Agency { AgencyId = "031" }, IpbaseNumber = "12345" } });
        ...
        /// </summary>
        [Fact]
        public async Task EL_01_EligibleAllIPBaseNumbersPDAndEligibleRoles()
        {
            var submission = new Submission() { Model = new SubmissionModel() };
            var rulesManagerMock = new Mock<IRulesManager>();
            var agreementManagerMock = new Mock<IAgreementManager>();
        ...
        /// </summary>
        [Fact]
        public async Task EL_01_InEligibleNotAllIPBaseNumbersPDAndEligibleRoles()
        {
            var submission = new Submission() { Model = new SubmissionModel() };
            var rulesManagerMock = new Mock<IRulesManager>();
            var agreementManagerMock = new Mock<IAgreementManager>();
        ...
        /// </summary>
        [Fact]
        public async Task EL_01_InEligibleNotAllIPBaseNumbersPD()
        {
            var submission = new Submission() { Model = new SubmissionModel() };
            var rulesManagerMock = new Mock<IRulesManager>();
            var agreementManagerMock = new Mock<IAgreementManager>();
        ...
        /// </summary>
        [Fact]
        public async Task EL_01_EligibleOneIpRepresentedBySubmitterChildAgency()
        {
            var submission = new Submission() { Model = new SubmissionModel() };
            var rulesManagerMock = new Mock<IRulesManager>();
            var agreementManagerMock = new Mock<IAgreementManager>();
