...
def test_CMQ_get_record():
    ...
    cmq_records = CMQ(api_response.json(), None, None).get_record()
...
def test_CMQ_get_child_records():
    api_response = Response()
    ...
    cmq_childrecords = CMQ(api_response.json(), {
                           'ParentIswc': ''}, None).get_child_records()

    expected_value = ['CWI                01000000880637991      010T9031153170           ',
                      'CIP                463130289  I-002234809-8C ']

    assert expected_value == cmq_childrecords
