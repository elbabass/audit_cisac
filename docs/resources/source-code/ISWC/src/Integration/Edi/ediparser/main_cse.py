...
from ediparser.parser.services.reporting_service import ReportingService
...
try:
    ...
    reporting_service = ReportingService(spark, logger, sftp)

    for (agency_id, file_type, file_header, file_contents) in reporting_service.get_agencies():
        try:
            (cse_file_name, cse_file_contents) = parser.run_cse(
                file_type, None, file_header, file_contents)

            sftp.put_cse_file(agency_id, cse_file_name, cse_file_contents)
        except:
            logger.log_exception(
                'Error: Creating CSE files for {}'.format(agency_id))
    sftp.close()
