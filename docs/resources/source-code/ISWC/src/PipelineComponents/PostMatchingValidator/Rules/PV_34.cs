...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.PostMatchingValidator.Rules
{
    public class PV_34 : IRule, IAlwaysOnRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            {
                var metadataMatch = submission.MatchedResult.Matches.FirstOrDefault().Numbers.FirstOrDefault(n => n.Type == "ISWC").Number;
                var isrcMatch = submission.IsrcMatchedResult.Matches.FirstOrDefault().Numbers.FirstOrDefault(n => n.Type == "ISWC").Number;
                if (!metadataMatch.Equals(isrcMatch))
                {
