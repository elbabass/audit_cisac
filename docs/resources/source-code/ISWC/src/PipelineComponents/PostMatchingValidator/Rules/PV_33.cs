    ...
    public class PV_33 : IRule, IAlwaysOnRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            if (submission.SearchWorks.Any())
            {
                ...
                InterestedPartyModel? selectedIp = null;
                if (subIp != null && matchingIps.Any())
                {
                    ...
                    if (ipBaseNumbers.Any())
                    {
                        ...
                        {
                            var filteredIps = interestedParties.Where(ip =>
                            ...
                            if (selectedIp != null)
                            {
                                ...
                                submission.Model.InterestedParties = new List<InterestedPartyModel> { selectedIp };

                                submission.SearchedIswcModels = submission.SearchedIswcModels
                                    .Where(iswc =>
                                        iswc.VerifiedSubmissions.Any(vs =>
                                            vs.InterestedParties.Any(ip => ip.IpBaseNumber == selectedIp.IpBaseNumber)))
