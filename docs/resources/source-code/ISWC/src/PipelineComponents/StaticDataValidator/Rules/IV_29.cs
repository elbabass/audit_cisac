...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.StaticDataValidator.Rules
{
    public class IV_29 : IRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            {
                if (ipsToReject.SelectMany(x => x.Names).Any(i => interestedParty.Names.Any(x => x.IpNameNumber == i.IpNameNumber))
                    || ipsToReject.Any(x => x.IpBaseNumber == interestedParty.IpBaseNumber))
                {
