...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.StaticDataValidator.Rules
{
    public class IV_19 : IRule
    {
		...
		public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            if (((model.InterestedParties == null) || (model.InterestedParties.SelectMany(x => x.Names).Any(n => n.IpNameNumber <= 0 || n.IpNameNumber == 0))) && submission.RequestType != RequestType.Label)
            {
