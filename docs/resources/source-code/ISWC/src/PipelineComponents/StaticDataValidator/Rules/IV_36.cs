...
{
    public class IV_36 : IRule
    {
        private readonly IMessagingManager messagingManager;
        private readonly IWorkManager workManager;
        private readonly IRulesManager rulesManager;

        public IV_36(IWorkManager workManager, IRulesManager rulesManager, IMessagingManager messagingManager)
        {
        ...
        public string Identifier => nameof(IV_36);
        public string ParameterName => "ValidateComposite";
        ...
        {
            var paramValue = await rulesManager.GetParameterValue<bool>("ValidateComposite");
            ...
            if (model.DerivedWorkType == DerivedWorkType.Composite)
            {
                if (!model.DerivedFrom.Any())
                {
                    submission.Rejection = await messagingManager.GetRejectionMessage(ErrorCode._121);
