...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.MetadataStandardizationValidator.Rules
{
    public class MD_16 : IRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            var ipNameNumbers = model.InterestedParties.SelectMany(ip => ip.Names.Select(x => x.IpNameNumber));
            ...
            if (submission.TransactionType == TransactionType.CIQ)
            {
                ...
                if (submission.RequestType == RequestType.ThirdParty && !model.InterestedParties.All(x => !x.IPNameNumber.HasValue))
                {
                    ...
                    {
                        var ipWithNameNumber = allIpsNameNumber.FirstOrDefault(x => interestedParty.Names.Any(y => x.Names.Any(z => z.IpNameNumber == y.IpNameNumber)));
            ...
            foreach (var interestedParty in model.InterestedParties.ToList())
            {
                ...
                var ipWithStatus = allIpsWithStatuses.FirstOrDefault(x => interestedParty.Names.Any(y => x.Names.Any(z => z.IpNameNumber == y.IpNameNumber)));
