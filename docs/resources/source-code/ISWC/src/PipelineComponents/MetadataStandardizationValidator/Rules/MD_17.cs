...
namespace SpanishPoint.Azure.Iswc.PipelineComponents.MetadataStandardizationValidator.Rules
{
    public class MD_17 : IRule
    {
        ...
        public async Task<(bool IsValid, Submission Submission)> IsValid(Submission submission)
        {
            ...
            if (paramValue)
            {
                var pgIpNameNumbers = await interestedPartyManager.CheckForPgIps(model.InterestedParties.SelectMany(ip => ip.Names.Select(x => x.IpNameNumber)));
                ...
                if (pgIps != null && pgIps.Any())
                {
                    ...
                    {
                        if (pgIps.Any(x => contributor.Names.Any(y => x.Names.Any(z => z.IpNameNumber == y.IpNameNumber))) && (submission.UpdateAllocatedIswc ? submission.AgencyToReplaceIasAgency : submission.Model.Agency) != null)
                        {
                            contributor.IsAuthoritative = submission.IsEligible ?
                                await interestedPartyManager.IsAuthoritative(contributor, eligibleAgencies) : false;
                            contributor.Names.FirstOrDefault().TypeCode = NameType.PG;

                            foreach (var pgIp in pgIps.Where(x => contributor.Names.Any(y => x.Names.Any(z => z.IpNameNumber == y.IpNameNumber))))
                            {
                                if (!model.InterestedParties.Select(ip => ip.IPNameNumber).Contains(pgIp.Names.FirstOrDefault(x => x.TypeCode == NameType.PA).IpNameNumber))
                                {
                                    var ipToAdd = new InterestedPartyModel()
                                    {
                                        ...
                                        IPNameNumber = pgIp.Names.FirstOrDefault(x => x.TypeCode == NameType.PA).IpNameNumber,
                                        Names = pgIp.Names.Where(x => x.TypeCode == NameType.PA).ToList(),
