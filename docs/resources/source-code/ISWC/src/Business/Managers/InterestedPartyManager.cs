    ...
    public class InterestedPartyManager : IInterestedPartyManager
    {
        ...
        public async Task<InterestedPartyModel> FollowChainForStatus(InterestedPartyModel interestedParty)
        {
            ...
            async Task<InterestedPartyModel> Recursive(InterestedPartyModel parentModel)
            {
                ...
                if (interestedParty.IpBaseNumber == child.IpbaseNumber)
                    return parentModel;

                if (child.Status.Any(x => x.StatusCode == (int)IpStatus.DeletionCrossReference))
                {
                    if (child.IpbaseNumber.Trim().Equals(child.Status.FirstOrDefault(s => s.FromDate <= now && s.ToDate > now).ForwardingBaseNumber.Trim()))
                        return mapper.Map<InterestedPartyModel>(child);
                    else
                    {
                        var childModel = mapper.Map<InterestedPartyModel>(child);
                        childModel.Status = mapper.Map<StatusModel>(child.Status.FirstOrDefault(x => x.StatusCode == (int)IpStatus.DeletionCrossReference));
