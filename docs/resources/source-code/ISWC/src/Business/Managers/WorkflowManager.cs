...
namespace SpanishPoint.Azure.Iswc.Business.Managers
{
    ...
    public class WorkflowManager : IWorkflowManager
    {
        ...
        public async Task<IEnumerable<Bdo.Work.WorkflowTask>> FindWorkflows(WorkflowSearchModel searchModel)
        {
            ...
            var updateIswcs = workflows.Where(x => x.WorkInfo != null).ToDictionary(x => x.WorkflowInstanceId, x => x.WorkInfo.IswcId);
            var mergeIswcs = workflows.Where(x => x.MergeRequest != null).ToDictionary(x => x.WorkflowInstanceId, x => x.MergeRequest.IswclinkedTo.FirstOrDefault()?.IswcId ?? 0);
            var demergeIswcs = workflows.Where(x => x.DeMergeRequest != null).ToDictionary(x => x.WorkflowInstanceId, x => x.DeMergeRequest.IswclinkedTo.FirstOrDefault()?.IswcId ?? 0);

            var workflowIswcs = updateIswcs.Union(mergeIswcs).Union(demergeIswcs).ToDictionary(x => x.Key, x => x.Value);
            var iswcModels = (await iswcRepository.GetIswcModels(workflowIswcs.Select(x => x.Value).Distinct(), true, DetailLevel.Core)).ToDictionary(x => x.IswcId, x => x);
        ...
        async Task<IswcModel> GetIswcMetadataForTask(WorkflowTask workflowTask)
        {
            ...
            if (workflowTask.WorkflowInstance?.WorkInfoId != null && result.VerifiedSubmissions.Any())
            {
                ...
                {
                    result.VerifiedSubmissions.FirstOrDefault(x => x.WorkInfoID == workflowTask.WorkflowInstance.WorkInfoId)
                };
