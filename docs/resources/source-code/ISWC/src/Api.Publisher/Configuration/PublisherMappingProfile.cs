...
namespace SpanishPoint.Azure.Iswc.Api.Publisher.Configuration
{
    internal class PublisherMappingProfile : Profile
    {
        public PublisherMappingProfile()
        {
            ...
            CreateMap<Submission, V1.IPContextSearchResponse>()
                .AfterMap((src, dest) =>
                {
                    if (src.Rejection != null)
                        return;

                    var ip = src.Model.InterestedParties.FirstOrDefault();

                    if (ip != null)
                    {
                        dest.NameNumber = ip.IPNameNumber
                            ?? ip.Names?.FirstOrDefault()?.IpNameNumber;

                        var name = ip.Names?.FirstOrDefault(n => n.IpNameNumber == dest.NameNumber);
                        dest.FirstName = name?.FirstName ?? string.Empty;
                        dest.LastName = name?.LastName ?? string.Empty;
                    }
                    else
                    {
                    ...
                    var matchingWorks = new List<V1.MatchingWork>();
                    if (src.SearchedIswcModels.Any())
                    {
                        foreach (var work in src.SearchedIswcModels)
                        {
                            ...
                            matchingWorks.Add(matchingWork);
                        }

                        if (matchingWorks.Count > 0)
                            dest.MatchingIswcs = matchingWorks;
                    }
                });
            ...
            .ForMember(dest => dest.SearchId, opt => opt.MapFrom(src => src.SubmissionId))
            .ForMember(dest => dest.Rejection, opt => opt.MapFrom(src => src.Rejection))
            .ForMember(dest => dest.SearchResult, opt => opt.MapFrom(src => src));
        }
    }
}
