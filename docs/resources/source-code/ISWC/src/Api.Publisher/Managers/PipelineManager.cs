...
namespace SpanishPoint.Azure.Iswc.Api.Publisher.Managers
{
    internal class PipelineManager : IPipelineManager
    {
        ...
        public async Task<IEnumerable<Submission>> RunPipelines(IEnumerable<Submission> submissions)
        {
            try
            {
                ...
                if (submissions.Any(x => x.SearchWorks.Any()))
                    ExplodeSearchWorks();

                var parentSubmissions = submissions.Where(x => x.SearchWorks.Any());
                submissions = await validationPipeline.RunPipeline(submissions.Where(x => !x.SearchWorks.Any()));
                submissions = await matchingPipeline.RunPipeline(submissions.Where(x => !x.SearchWorks.Any()));
                submissions = submissions.Concat(parentSubmissions);

                if (submissions.Any(x => x.MultipleAgencyWorkCodes.Any()))
                    SortForPostMatcing();

                if (submissions.Any(x => x.SearchWorks.Any()))
                    SortSearchForPostMatcing();
                ...
                void ExplodeSearchWorks()
                {
                    var subsToAdd = new List<Submission>();
                    var subId = submissions.Last().SubmissionId;
                    foreach (var submission in submissions.Where(x => x.SearchWorks.Any()))
                    {
                        foreach (var x in submission.SearchWorks)
                            subsToAdd.Add(CreateChildSearchSubmission(submission, x, subId += 1));
                    }
                        ...
                        foreach (var parentSubmission in parentSubmissions)
                        {
                            ...
                            foreach (var childSubmission in childSubmissions)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (parentSubmission.SearchedIswcModels == null)
                                            parentSubmission.SearchedIswcModels = new List<IswcModel>();
        ...
        {
            return (await RunPipelines(new List<Submission> { submission })).FirstOrDefault();
        }
        ...
        Submission CreateChildSearchSubmission(Submission parent, WorkModel work, int id)
        {
            var parentId = parent.SubmissionId;
            var child = new Submission();
            child.Model = new SubmissionModel();
            child.Model = parent.Model.Copy();
            child.Model.PreferredIswc = work.PreferredIswc;
            child.Model.Titles = work.Titles;
            child.Model.AdditionalAgencyWorkNumbers = work.WorkNumbers;
            child.SubmissionParentId = parentId;
            child.SubmissionId = id;
            child.RequestSource = parent.RequestSource;
            child.RequestType = parent.RequestType;
            child.DetailLevel = parent.DetailLevel;
            child.TransactionType = parent.TransactionType;
